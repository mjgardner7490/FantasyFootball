...
namespace YahooSports.OAuthLib.Core
{
  public class AuthorizeHeader
  {
    ...
    public AuthorizeHeader(string realm, string consumerKey, string signatureMethod, string signature, string timestamp, string nounce, string version, string token, string verifier, string sessionHandle = null)
      : this(realm, consumerKey, signatureMethod, signature, timestamp, nounce, version)
    {
      Token = token;
      Verifier = verifier;
    ...
    public override string ToString()
    {
      ...
      if (!String.IsNullOrEmpty(SessionHandle))
          sb.AppendFormat("{0}=\"{1}\", ", OAuthProtocolParameter.SessionHandle.GetStringValue(), OAuthUtils.UrlEncode(SessionHandle));
