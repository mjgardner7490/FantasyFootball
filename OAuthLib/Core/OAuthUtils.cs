...
namespace YahooSports.OAuthLib.Core
{
  ...
  public class OAuthUtils
  {
    ...
    private static string NormalizeProtocolParameters(IList<ProtocolParameter> parameters)
    {
      ...
      {
        p = parameters[i];
        sb.AppendFormat("{0}={1}", p.Name, UrlEncode(p.Value));//TODO: UrlEncode(p.Value).Replace(" ", "%20"));
    ...
    public static List<ProtocolParameter> ExtractQueryStrings(string url) {
            ...
            foreach (var part in parts) {
                if (!string.IsNullOrEmpty(part) && !part.StartsWith(OAuthProtocolParameterPrefix)) {
                    if (part.IndexOf('=') != -1) {
                        ...
                        for (int i = 2; i < nameValue.Length; i++) {
    ...
    public static string GenerateSignatureBaseString(string url, string httpMethod, List<ProtocolParameter> protocolParameters) {
        ...
        signatureBaseSb.AppendFormat("{0}", UrlEncode(normalizedRequestParameters).Replace(" ", "%20"));
        return signatureBaseSb.ToString();
    }

    public static string GenerateSignature(string consumerSecret, SignatureMethod signatureMethod, string signatureBaseString, string tokenSecret = null) {
        switch (signatureMethod) {
            ...
            case SignatureMethod.PLAINTEXT:
                return String.Format("{0}&{1}", UrlEncode(consumerSecret), String.IsNullOrEmpty(tokenSecret) ? "" : UrlEncode(tokenSecret));
  ...
  internal enum OAuthProtocolParameter
  {
    ...
    TokenSecret,
    [EnumStringValueAttribute("oauth_session_handle")]
    SessionHandle
  }
